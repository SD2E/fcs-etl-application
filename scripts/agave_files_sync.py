...
def credentials():
    ...
    # credential store
    if environ.get('AGAVE_CACHE_DIR', None) is not None:
        ag_cred_store = join(
            environ.get('AGAVE_CACHE_DIR'), 'current')
    else:
        ag_cred_store = expanduser('~/.agave/current')

    if exists(ag_cred_store):
        logger.debug('Reading {}'.format(ag_cred_store))
...
def get_path_type(path):
    ...
    if path[:len(agave_prefix)] == agave_prefix:
        ...
        assert isfile(path) or isdir(path),\
            'Invalid local path: {}'.format(path)
        path_type = LOCAL
    return path_type
...
def agave_path_builder(base, path, recursive=False):
    """
    Generates a ready-to-use agave url with the cached base
    and user-provided path
    """
    assert get_path_type(path) == AGAVE,\
        'Path is type {}, must be type agave'.format(get_path_type(path))
    # strip agave prefix
...
def agave_path_setlisting(path, base, listings=True):
    """
    Sets path prefix to /files/v2/listings/ when listings=True (default)
    and /files/v2/media/ when listings=False
    """
    if listings:
        path = path.replace('{}/files/v2/media'.format(base),
                            '{}/files/v2/listings'.format(base))
    else:
        path = path.replace('{}/files/v2/listings'.format(base),
                            '{}/files/v2/media'.format(base))
    return path


def sametype(local_filepath, agave_description):
    """
    Checks if local and agave files are both directories or
    files. Returns boolean.
    """
    return (isfile(local_filepath) and agave_description['type'] == 'file') \
        or (isdir(local_filepath) and agave_description['type'] == 'dir')
...
def list_agave_dir_files(url, headers):
    ...
    payload = {'limit': 300}
    r = get(url, headers=headers, params=payload)
